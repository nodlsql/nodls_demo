...
pub fn test_insert_key() {
    ...
    let test_data = [
        ...
        (("val1", 123), 111, true),
        (("val1", 123), 112, true), // duplicate key, should succeed
        (("val2", 456), 113, true),
    ];
...
fn test_delete_key() {
    ...
    dupindexkey::pretty_print_index_page(&serialized_page).unwrap();
    ...
    let delete_res = indexbt::delete_key(114, &del_key, &mut serialized_page, false);
    assert!(
        delete_res.is_ok(),
